...
class DELGforWA:
    def __init__(
        ...
        weight_factor: float = 0.5,  # Controla influencia de donador local y global
    ):
    ...
    def get_neighborhood(self, index: int) -> list:
        """Genera el vecindario de tamaño `self.neighborhood_size` alrededor del índice `index`."""
        return [
            (index + i) % self.num_vectors
    ...
    def optimize(self) -> Tuple[np.ndarray, float]:
        for i in range(self.max_iterations):
            ...
            for v, vector in enumerate(self.population):
                trial_vector = self.mutate_and_crossover(vector, v)
    ...
    def mutate_and_crossover(self, vector: Vector, index: int) -> Vector:
        # Vecindario del vector actual
        neighborhood_indices = self.get_neighborhood(index)
        neighborhood = [self.population[i] for i in neighborhood_indices]

        # Encontrar el mejor vector en el vecindario
        best_local_vector = max(neighborhood, key=lambda vec: vec.fitness)

        # Generar el donador local
        a, b = np.random.choice(neighborhood, 2, replace=False)
        donor_local = (
            vector.position
            + self.mutation_factor * (best_local_vector.position - vector.position)
            + self.mutation_factor * (a.position - b.position)
        )

        # Generar el donador global (target-to-best general)
        a_global, b_global = np.random.choice(self.population, 2, replace=False)
        donor_global = (
            vector.position
            + self.mutation_factor * (self.global_best_position - vector.position)
            + self.mutation_factor * (a_global.position - b_global.position)
        )

        # Vector donador conjunto
        donor_combined = (
            self.weight_factor * donor_global + (1 - self.weight_factor) * donor_local
        )

        # Crossover y generación del trial vector
        trial_vector_position = np.copy(vector.position)
        forced_index = np.random.randint(0, vector.position.shape[0] - 1)
        trial_vector_position[forced_index] = donor_combined[forced_index]
        crossover = np.random.rand(len(vector.position)) < self.crossover_probability
        trial_vector_position[crossover] = donor_combined[crossover]
...
def execute_on_all_datasets():
    ...
    with open("test/dengbh-v10-cr80-mf50-i150-ngbh2-ff1-clipped.out", mode="w") as file:
